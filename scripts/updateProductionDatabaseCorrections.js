...
if (!uri) {
    console.error('MONGODB_URI not found in .env.atlas');
    process.exit(1);
}
...
async function updateDatabase() {
    ...
    try {
        ...
        const seagate24Result = await db.collection('storage').updateOne(
        ...
        const wd4tbResult = await db.collection('storage').updateOne(
            {
        ...
        const seagate22Result = await db.collection('storage').updateOne(
        ...
        const sp4tbResult = await db.collection('storage').updateOne(
        ...
        const wd2tbResult = await db.collection('storage').updateOne(
            {
        ...
        const kingston2tbResult = await db.collection('storage').updateOne(
            { name: /SFYR2S\/2T0/i },
            { $set: { capacity: '2TB' } }
        );
        ...
        const kingston1tbResult = await db.collection('storage').updateOne(
            { name: /SFYR2S\/1T0/i },
            { $set: { capacity: '1TB' } }
        );
        ...
        const toshibaResult = await db.collection('storage').deleteMany({
            $and: [
                { brand: /toshiba/i },
                { name: /PS3\/PS4/i }
            ]
        ...
        const maxDigitalResult = await db.collection('storage').deleteOne({
            name: /MaxDigitalData.*PS3\/PS4/i
        });
        console.log(`   Deleted ${maxDigitalResult.deletedCount} MaxDigitalData drive(s)`);

        console.log('\n✅ All production database corrections completed!');
