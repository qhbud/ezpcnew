...
async function updateAllPriceHistory() {
    const collections = ['storages', 'psus', 'coolers', 'rams', 'motherboards', 'cpus', 'gpus'];
    const updater = new PriceUpdater();

    Logger.info('🔄 Starting price history update for all component types...\n');
    ...
    try {
        ...
        for (const collection of collections) {
            ...
            try {
                const collectionResults = await updater.updateMultipleItems(collection, { sourceUrl: { $exists: true, $ne: null } }, 999999);
...
module.exports = { updateAllPriceHistory };
